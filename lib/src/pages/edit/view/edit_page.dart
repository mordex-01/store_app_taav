...
class EditPage extends GetView<EditController> {
              ...
              children: [
                ...
                Padding(
                  ...
                  child: _addButton(
                    context: context,
                    onTap: () {
                      controller.onEditProductTapped(id: controller.args);
                    },
                  ),
                ),
              ],
        ...
        () => ListView.builder(
          ...
          itemBuilder: (context, index) => Container(
            ...
            decoration: BoxDecoration(
                ...
                border: Border.all(
                    width: 2, color: const Color.fromARGB(255, 0, 67, 121))),
            child: InkWell(
              onTap: () {
                for (var a in controller.tags) {
                  if (controller.allTags.toSet().toList()[index] == a) {
                    return;
                  }
                }
                controller.tags.add(controller.allTags.toSet().toList()[index]);
              },
              child: ListTile(
                leading: Text(
  ...
  Widget _tagsSection({required BuildContext context}) => Row(
        children: [
          Padding(
          ...
          IconButton(
            onPressed: () {
              if (controller.addTagsTextFieldFormKey.currentState!.validate()) {
                ...
                controller.tags.add(controller.tagTextFieldController.text);
                controller.tagTextFieldController.clear();
              }
            },
            icon: const Icon(
  ...
  Widget _colorsSection({required BuildContext context}) => Padding(
        padding: const EdgeInsets.all(8.0),
        child: Obx(
          () => Row(
  ...
  Widget _countTextField() => Padding(
        padding: const EdgeInsets.all(8.0),
        child: _textFormField(
          controller: controller.countController,
          validator: (value) {
            if (value == null || value.isEmpty) {
              return "Count Cant Be Empty";
            }
            return null;
          },
          maxLength: 10,
  ...
  Widget _priceTextField() => Padding(
      padding: const EdgeInsets.all(8),
      child: _textFormField(
          maxLength: 6,
          keyboardType: TextInputType.number,
  ...
  Widget _leadingIconButton({required void Function()? ontap}) => IconButton(
        onPressed: ontap,
